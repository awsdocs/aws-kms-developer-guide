...
A *grant token* is unique, nonsecret, variable\-length, base64\-encoded string that represents a grant\. You can use the grant token to identify the grant in any [grant operation](#terms-grant-operations)\. However, because the token value is a hash digest, it doesn't reveal any details about the grant\.  
