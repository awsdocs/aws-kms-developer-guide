...
You can create one or several [custom key stores](key-store-concepts.md#concept-custom-key-store) in your account\. Each custom key store resides in one AWS Region and it is associated with one AWS CloudHSM cluster that you own and manage\. Before you create your custom key store, you need to [assemble the prerequisites](#before-keystore)\. Then, before you can use your custom key store, you must [connect it](disconnect-keystore.md) to its AWS CloudHSM cluster\. 
...
+ **The cluster must be in the same account and Region** as the AWS KMS custom key store\.
