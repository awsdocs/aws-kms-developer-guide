...
+ Each [private subnet for the cluster](https://docs.aws.amazon.com/cloudhsm/latest/userguide/create-subnets.html) **must have at least one available IP address** \(two are preferable\) to connect the custom key store\. If any private subnet associated with the cluster is out of IP addresses, the connection fails with an [`INSUFFICIENT_FREE_ADDRESSES_IN_SUBNET`](fix-keystore.md#no-ips) connection error code\. Because you can't add IP addresses to an existing subnet, if you can't free up address space, you'll need to create a cluster from a backup with a different private subnet\.
