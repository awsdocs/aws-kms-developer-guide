...
When you update an alias, the current KMS key and the new KMS key must be the same type \(both symmetric or both asymmetric\)\. They must also have the same key usage \(`ENCRYPT_DECRYPT` or `SIGN_VERIFY`\)\. This restriction prevents cryptographic errors in code that uses aliases\. 
