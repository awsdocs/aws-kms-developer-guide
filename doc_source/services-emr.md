...
For both server\-side and client\-side encryption, the KMS key you choose is the root key in an [envelope encryption](concepts.md#enveloping) workflow\. The data is encrypted with a unique [data key](concepts.md#data-keys) that is encrypted under the KMS key in AWS KMS\. The encrypted data and an encrypted copy of its data key are stored together as a single encrypted object in an S3 bucket\. For more information about how this works, see the following topics\.
